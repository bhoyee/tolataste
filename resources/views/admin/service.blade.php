                                    ...
                                    <tr>
                                        ...
                                        <td>
                                          @forelse ($languages as $language)
                                            <a href="{{ route('admin.translation.service.create', [
                                            'code' => $language->code,
                                            'id' => $service->id,
                                          ]) }}"><i class="fa {{ $service?->translation($language->code)?->first()?->title ? 'fa-check' : 'fa-edit' }}"></i> {{ strtoupper($language->code) }}</a>
                                            @if(!$loop->last)
                                              | 
                                            @endif
                                          @empty
                                          <a href="{{ route('admin.translation.service.create', [
                                            'code' => app()->getLocale(),
                                            'id' => $service->id,
                                          ]) }}"><i class="fa {{ $service?->translation(app()->getLocale())?->first()?->title ? 'fa-check' : 'fa-edit' }}"></i> {{ strtoupper(app()->getLocale()) }}</a>
                                          @endforelse
                                        </td>
                                        <td>
                                            @if($service->status == 1)
