...
@section('admin-content')
                        ...
                        <table class="table table-striped" id="dataTable">
                            ...
                            <tbody>
                                @foreach ($reservations as $index => $reservation)
                                    <tr>
                                        <td>{{ ++$index }}</td>
                                        <td class="table_user_info">
                                          @if ($reservation->user)
                                              <p>
                                                  <a href="{{ route('admin.customer-show', $reservation->user_id) }}">
                                                      {{ $reservation->user->name }}
                                                  </a>
                                              </p>
                                          @else
                                          <p>
                                            <a href="javascript:;" data-toggle="modal" data-target="#guestDetailModal-{{ $reservation->id }}">
                                              <strong>{{ $reservation->guest_name }}</strong> <span class="badge badge-info">Guest</span>
                                            </a>
                                          </p>
      ...
      @if (!$reservation->user)
  <div class="modal fade" id="guestDetailModal-{{ $reservation->id }}" tabindex="-1" role="dialog" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
              <div class="modal-header">
                  <h5 class="modal-title">Guest Reservation Details</h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                      <span aria-hidden="true">&times;</span>
                  </button>
              </div>
