                                    ...
                                    <tr>
                                        ...
                                        <td>
                                          @forelse ($languages as $language)
                                            <a href="{{ route('admin.translation.product.create', [
                                            'code' => $language->code,
                                            'id' => $product->id
                                          ]) }}"><i class="fa {{ $product?->translation($language->code)?->first()?->name ? 'fa-check' : 'fa-edit' }}"></i> {{ strtoupper($language->code) }}</a>
                                            @if(!$loop->last)
                                              | 
                                            @endif
                                          @empty
                                          <a href="{{ route('admin.translation.product.create', [
                                            'code' => app()->getLocale(),
                                            'id' => $product->id
                                          ]) }}"><i class="fa {{ $product?->translation(app()->getLocale())?->first()?->name ? 'fa-check' : 'fa-edit' }}"></i> {{ strtoupper(app()->getLocale()) }}</a>
                                          @endforelse
                                        </td>
                                        ...
                                        <div class="dropdown d-inline">
                                            ...
                                            <div class="dropdown-menu" x-placement="top-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(0px, -131px, 0px);">
                                              ...
                                              <a class="dropdown-item has-icon" href="{{ route('admin.product-variant',$product->id) }}"><i class="fas fa-cog"></i>{{__('admin.Extra / Optional Item')}}</a>
