                        ...
                        <table class="table table-striped" id="dataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th width="55%">{{__('admin.Translations')}}</th>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                          @forelse ($languages as $language)
                                            <a href="{{ route('admin.translation.faq.create', ['code' => $language->code, 'id' => $faq->id]) }}"><i class="fa {{ $faq->translation($language->code)?->first()?->answer ? 'fa-check' : 'fa-edit' }}"></i>
                                                {{ strtoupper($language->code) }}</a>
                                            @if(!$loop->last)
                                            |
                                            @endif
                                            @empty
                                            <a href="{{ route('admin.translation.faq.create', [
                                          'code' => app()->getLocale(), 'id' => $faq->id
                                      ]) }}"><i class="fa {{ $faq->translation(app()->getLocale())->first()?->answer ? 'fa-check' : 'fa-edit' }}"></i>
