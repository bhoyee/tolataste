...
function initAutocomplete() { 
    ...
    suggestionBox.id = "address_suggestions";
    ...
    input.addEventListener('input', function () {
        ...
        timeout = setTimeout(async () => {
            ...
            try {
                // ✅ Clean raw address before building query
                const cleanedInput = cleanAddressForSearch(raw);

                // Try structured query
                const q1 = buildStructuredQuery(cleanedInput);
                let results = await nominatimSearch(q1.url);

                // If empty, try fallback with Maryland
                if (!results.length) {
                    const q2 = `${cleanedInput}, Maryland, USA`;
                    results = await nominatimSearch(
                        `https://nominatim.openstreetmap.org/search?format=json&addressdetails=1&limit=5&countrycodes=us&q=${encodeURIComponent(q2)}`
                    );
                }

                // Last fallback: raw cleaned query
                if (!results.length) {
                    results = await nominatimSearch(
                        `https://nominatim.openstreetmap.org/search?format=json&addressdetails=1&limit=5&countrycodes=us&q=${encodeURIComponent(cleanedInput)}`
                    );
                }
                ...
                if (!results.length) {
                    toastr.error("❌ Address not found. Try without Apt/Suite number.");
                    return;
                }
                ...
                loader.style.display = 'none';
                toastr.error("❌ Address search failed.");
            }
        }, 400);
    });
    ...
    function isMaryland(addr) {
        const state = (addr.state || '').toLowerCase();
        const code = (addr.state_code || '').toUpperCase();
        return state === 'maryland' || code === 'MD';
    }
    ...
    async function onPlaceSelected(place) {
        ...
        try {
            const res = await fetch('https://api.openrouteservice.org/v2/matrix/driving-car', {
                ...
                body: JSON.stringify({
                    locations: [[storeLng, storeLat], [lon, lat]],
                    metrics: ['distance'],
                    units: 'mi'
                })
            });
    ...
    // --- Helper functions ---
    function cleanAddressForSearch(raw) {
        return raw
            .replace(/\b(Suite|Ste|Unit|Apt|#)\s*\d+[A-Za-z0-9-]*/gi, '') // remove suite/unit/apt
            .replace(/\s+/g, ' ')
            .trim();
    }

    function normalizeUnits(raw) {
        const m = raw.match(/^(.*?\d+\s+[A-Za-z].*?|.*?Rd|.*?St|.*?Ave|.*?Blvd|.*?Ln|.*?Dr)\s+([\dA-Za-z ]{2,})$/i);
        if (!m) return raw;
        const base = m[1].trim();
    ...
    function buildStructuredQuery(rawInput) {
        const raw = rawInput.trim();
        const cleaned = normalizeUnits(raw);

        const zipMatch = cleaned.match(/\b(\d{5})(?:-\d{4})?\b/);
        const postal = zipMatch ? zipMatch[1] : '';

        const parts = cleaned.split(',').map(s => s.trim());
        let street = '', city = '', state = 'MD', country = 'USA';

        if (parts.length >= 1) street = parts[0];
        if (parts.length >= 2) city = parts[1];
        if (parts.length >= 3 && parts[2].match(/MD|Maryland/i)) state = 'MD';

        if (!city && cleaned.toLowerCase().includes('twin knolls')) {
            city = 'Columbia';
        }
        ...
        return { url: `https://nominatim.openstreetmap.org/search?${params.toString()}` };
    }
