...
class ProductController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.product', compact('products', 'orderProducts', 'setting', 'frontend_view', 'languages'));
    }
   ...
   public function store(Request $request)
{
    ...
    if ($request->thumb_image) {
        ...
        $generatedName = Str::slug($request->name) . date('-Y-m-d-h-i-s-') . rand(999, 9999) . '.' . $ext;
        ...
        Image::make($request->thumb_image)->save($absolutePath);
        $product->thumb_image = $relativePath;
    }
...
if ($request->thumb_image) {
    ...
    if ($old_thumbnail && File::exists(base_path('../public_html/' . $old_thumbnail))) {
        unlink(base_path('../public_html/' . $old_thumbnail));
    }
}
