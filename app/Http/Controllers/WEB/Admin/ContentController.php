...
public function updateDefaultAvatar(Request $request)
{
    ...
    if ($request->hasFile('avatar')) {
        ...
        // Get the correct absolute path to public_html
        $destination = base_path('../public_html/uploads/website-images/');
        if (!file_exists($destination)) {
            mkdir($destination, 0755, true);
        }

        $filename = 'default-avatar-' . date('Y-m-d-H-i-s') . '.' . $image->getClientOriginalExtension();
        $image->move($destination, $filename);

        // Optional: Delete old avatar
        if ($setting->default_avatar && file_exists(base_path('../public_html/' . $setting->default_avatar))) {
            @unlink(base_path('../public_html/' . $setting->default_avatar));
        }

        $setting->default_avatar = 'uploads/website-images/' . $filename;
        $setting->save();
    }

    return redirect()->back()->with([
        'messege' => __('admin.Default avatar updated successfully.'),
        'alert-type' => 'success'
    ]);
}
    ...
    public function app_section()
    {
        ...
        return view('admin.app_section', compact('app_section', 'languages'));
    }
