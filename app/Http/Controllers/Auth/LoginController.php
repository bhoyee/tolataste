...
public function send_reset_link(Request $request){
    \Log::info('Forget password attempt for: ' . $request->email);
    ...
    try {
        ...
        if($user){
            ...
            // REMOVE MailHelper and use direct config
            // MailHelper::setMailConfig(); // COMMENT THIS OUT
            ...
            \Log::info('Attempting to send email with: ' . config('mail.mailers.smtp.username'));
            
            Mail::to($user->email)->send(new UserForgetPassword($message,$subject,$user));

            $notification = trans('Reset password link sent to your email.');
            return redirect()->back()->with(['messege'=>$notification,'alert-type'=>'success']);

        } else {
            \Log::warning('Email not found in database: ' . $request->email);
            $notification = trans('Email does not exist');
            return redirect()->route('forget-password')->with(['messege'=>$notification,'alert-type'=>'error']);
        }
    } catch (\Exception $e) {
        \Log::error('Password reset general error: ' . $e->getMessage());
        $notification = trans('An error occurred. Please try again.');
